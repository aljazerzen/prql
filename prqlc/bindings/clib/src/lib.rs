...
use prqlc_main::ErrorMessages;
use prqlc_main::Target;
use std::ffi::CStr;
use std::ffi::CString;
use std::str::FromStr;
    ...
    let result = options
        .and_then(|opts| {
            Ok(prql_query.as_str())
                .and_then(prqlc_main::prql_to_pl)
                .and_then(prqlc_main::pl_to_rq)
                .and_then(|rq| prqlc_main::rq_to_sql(rq, &opts.unwrap_or_default()))
        })
...
/// [prqlc_main Rust crate](https://docs.rs/prqlc_main/latest/prql_compiler/ast/pl).
///
...
pub unsafe extern "C" fn prql_to_pl(prql_query: *const c_char) -> CompileResult {
    ...
    let result = Ok(prql_query.as_str())
        .and_then(prqlc_main::prql_to_pl)
        .and_then(prqlc_main::json::from_pl);
    result_into_c_str(result)
}
...
/// [prqlc_main Rust crate](https://docs.rs/prqlc_main/latest/prql_compiler/ast).
///
...
pub unsafe extern "C" fn pl_to_rq(pl_json: *const c_char) -> CompileResult {
    ...
    let result = Ok(pl_json.as_str())
        .and_then(prqlc_main::json::to_pl)
        .and_then(prqlc_main::pl_to_rq)
        .and_then(prqlc_main::json::from_rq);
    result_into_c_str(result)
}

/// Convert RQ AST into an SQL string. RQ is documented in the
/// [prqlc_main Rust crate](https://docs.rs/prqlc_main/latest/prql_compiler/ast/rq).
///
...
pub unsafe extern "C" fn rq_to_sql(
    ...
    let result = options.and_then(|options| {
        Ok(rq_json.as_str())
            .and_then(prqlc_main::json::to_rq)
            .and_then(|x| prqlc_main::rq_to_sql(x, &options.unwrap_or_default()))
    });
    result_into_c_str(result)
}
...
// Make sure to keep in sync with prqlc_main::ErrorMessage
#[repr(C)]
pub struct Message {
...
// Make sure to keep in sync with prqlc_main::Span
#[repr(C)]
pub struct Span {
    pub start: size_t,
    pub end: size_t,
...
// Make sure to keep in sync with prqlc_main::SourceLocation
#[repr(C)]
...
fn convert_span(x: prqlc_main::Span) -> Span {
    Span {
        start: x.start,
        end: x.end,
    }
...
fn convert_source_location(x: prqlc_main::SourceLocation) -> SourceLocation {
    SourceLocation {
...
fn convert_options(o: &Options) -> Result<prqlc_main::Options, prqlc_main::ErrorMessages> {
    ...
    let target = Target::from_str(target).map_err(|e| prqlc_main::downcast(e.into()))?;

    Ok(prqlc_main::Options {
        format: o.format,
        target,

...
fn to_result_tuple(result: Result<String, prqlc_main::ErrorMessages>) -> NifResult<Response> {
    match result {
        Ok(sql) => Ok(Response {
            status: atoms::ok(),
            result: sql,
...
fn target_from_atom(a: Atom) -> prqlc_main::Target {
    use prqlc_main::sql::Dialect::*;

    prqlc_main::Target::Sql(Some(if a == atoms::ansi() {
        Ansi
...
impl From<CompileOptions> for prqlc_main::Options {
    /// Get `prqlc_main::Options` options from `CompileOptions`
...
pub fn compile(prql_query: &str, options: CompileOptions) -> NifResult<Response> {
    to_result_tuple(prqlc_main::compile(prql_query, &options.into()))
}
...
pub fn rq_to_sql(rq_json: &str) -> NifResult<Response> {
    to_result_tuple(
        Ok(rq_json)
            ...
            .and_then(|x| prqlc_main::rq_to_sql(x, &prqlc_main::Options::default())),
    )
}

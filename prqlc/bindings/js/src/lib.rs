...
pub fn compile(prql_query: &str, options: Option<CompileOptions>) -> Option<String> {
    return_or_throw(
        prqlc_main::compile(prql_query, &options.map(|x| x.into()).unwrap_or_default())
...
fn return_or_throw(result: Result<String, prqlc_main::ErrorMessages>) -> Option<String> {
    match result {
        Ok(sql) => Some(sql),

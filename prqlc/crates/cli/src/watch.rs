...
pub fn run(command: &mut WatchArgs) -> Result<()> {
    let opt = prqlc_main::Options {
        format: !command.no_format,
        target: prqlc_main::Target::Sql(None),
...
fn find_and_compile(path: &Path, opt: &prqlc_main::Options) -> Result<()> {
...
fn watch_and_compile(path: &Path, opt: &prqlc_main::Options) -> Result<()> {
...
fn compile_path(path: &Path, opt: &prqlc_main::Options) -> Result<()> {
    ...
    let sql_string = match prqlc_main::compile(&prql_string, opt) {

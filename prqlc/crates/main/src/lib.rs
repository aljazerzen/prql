...
//!
//!     ```
//!     # fn main() -> Result<(), prqlc_main::ErrorMessages> {
//!     let sql = prqlc_main::compile(
//!         "from albums | select {title, artist_id}",
//!          &prqlc_main::Options::default().no_format()
//!     )?;
...
//!
//!     For inline strings, use the `prqlc_macros` crate; for example:
//!     ```ignore
//!     let sql: &str = prql_to_sql!("from albums | select {title, artist_id}");
//!     ```
//!
//!     For compiling whole files (`.prql` to `.sql`), call `prqlc_main`
//!     from `build.rs`.
...
pub static COMPILER_VERSION: Lazy<Version> = Lazy::new(|| {
    Version::parse(env!("CARGO_PKG_VERSION")).expect("Invalid prqlc_main version number")
});
...
pub fn pl_to_rq(pl: Vec<prqlc_ast::stmt::Stmt>) -> Result<ir::rq::Query, ErrorMessages> {

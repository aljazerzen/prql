...
use prqlc_main::{sql::Dialect, sql::SupportLevel, Target::Sql};
use prqlc_main::{Options, Target};

mod connection;

// This is copy-pasted from `test.rs` in prqlc_main. Ideally we would have a
...
fn compile(prql: &str, target: Target) -> Result<String, prqlc_main::ErrorMessages> {
    prqlc_main::compile(prql, &Options::default().no_signature().with_target(target))
}
...
fn test_fmt_examples() {
    glob!("queries/**/*.prql", |path| {
        ...
        let pl = prqlc_main::prql_to_pl(&prql).unwrap();
        let formatted = prqlc_main::pl_to_prql(pl).unwrap();
...
fn test_rdbms() {
    ...
    glob!("queries/**/*.prql", |path| {
        ...
        for con in &mut connections {
            ...
            let mut rows = prqlc_main::compile(&prql, &options)
